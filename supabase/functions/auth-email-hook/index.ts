...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  const subject = EMAIL_SUBJECTS[emailType] || 'Notificação — VBUCKS BARATO'

  // Send directly via Resend
  const result = await sendViaResend(payload.data.email, subject, html)

  if (!result.success) {
    console.error('Failed to send auth email via Resend', { emailType, error: result.error, run_id })
